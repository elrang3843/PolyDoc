...
### Fixed
- **Fixed** — 왼쪽으로 진행(RTL) 모드에서 글자 입력 시 초기 문자들이 보이지 않고 4자 이상 입력해야 나타나는 버그. `RichTextBox`(컨테이너) 의 `FlowDirection` 만 변경하고 내부 `FlowDocument.FlowDirection` 을 LTR 로 방치해 두 방향이 충돌, RTL 텍스트 시작점이 스크롤 뷰포트 밖에 배치되던 원인. 컨테이너·FlowDocument 양쪽 모두 `FlowDirection` 을 동기화해 수정. `TextBoxOverlay` 글상자와 `BodyEditor` 메인 편집기 모두 동일하게 적용.
