...
### Fixed
- **Fixed** — **그림 속성 다이얼로그에서 "오른쪽 배치" 선택 시 그림 사라지던 버그** (2차 수정). 1차 수정에서 `FontSize=0.1` → `LineHeight=0.1` + `Run("")` 로 바꿨으나, WPF TextFormatter 는 빈 문자열(`""`) Run 을 글리프 없음으로 처리해 라인을 "유효한 라인" 으로 인식하지 않고 우측 정렬 Floater 위치를 결정하지 못해 그림이 사라지는 현상이 지속. anchor Run 의 문자를 비줄바꿈 공백(U+00A0) 으로 교체 — 실제 폭 있는 글리프를 생성해 WPF 가 라인을 유효하게 처리하도록 강제. `Foreground=Transparent` 로 U+00A0 는 시각적으로 보이지 않음.
