...
### Fixed
- **Fixed** — **그림 속성 다이얼로그에서 "오른쪽 배치" 선택 시 그림 사라지던 버그**. 원인: WrapLeft/WrapRight 플레이스홀더 단락의 `FontSize=0.1` 가 라인 폭 계산을 0 으로 만들어 우측 정렬 Floater 가 자기 가용 너비를 0 으로 인식하고 렌더링되지 않았음. 단락에 폰트 크기 대신 `LineHeight=0.1` 을 적용하고 Floater 뒤에 빈 anchor Run 을 두어 라인 폭이 보존되도록 수정.
