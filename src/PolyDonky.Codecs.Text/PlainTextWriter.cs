...
namespace PolyDonky.Codecs.Text;

/// <summary>PolyDonkyument 의 모든 문단을 LF 로 이어 단일 평문으로 직렬화한다.</summary>
public sealed class PlainTextWriter : IDocumentWriter
{
    ...
    public static string ToText(PolyDonkyument document, string newLine = "\n")
    {
        ArgumentNullException.ThrowIfNull(document);
    ...
    private static void WriteTo(PolyDonkyument document, StreamWriter writer)
    {
        bool first = true;
        foreach (var paragraph in document.EnumerateParagraphs())
        {
