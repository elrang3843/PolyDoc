...
// PolyDonky.Convert.Doc — DOC ↔ IWPF 변환 전용 콘솔 도구.
// CLAUDE.md §3 의 외부 변환 모듈 분리 원칙: 메인 앱은 IWPF/MD/TXT 만 직접 처리하고
// DOC 는 이 CLI 가 처리한다.
//
// 변환 파이프라인:
//   *.doc → *.iwpf : LibreOffice headless (doc→docx) → DocxReader → IwpfWriter
//   *.iwpf → *.doc : IwpfReader → DocxWriter → LibreOffice headless (docx→doc)
//
// 사용법:
...
if (args.Length != 2)
{
    ...
    Console.Error.WriteLine("  Supported: .doc <-> .iwpf");
...
if (sOffice is null)
{
    ...
    return ConverterExitCodes.OldVersion;  // 6 = 지원 범위 밖 버전 (LibreOffice 없음)
}
...
// 임시 작업 디렉터리 (LibreOffice 출력용)
var tmpDir = Path.Combine(Path.GetTempPath(), $"polydonky-doc-{Guid.NewGuid():N}");
...
try
{
    if (isImport)
    {
        // DOC → (LibreOffice) → 임시 DOCX → (DocxReader) → IWPF
        ConverterProgress.Write(0, "LibreOffice 로 DOC → DOCX 변환 중");
    ...
    {
        // IWPF → (IwpfReader) → (DocxWriter) → 임시 DOCX → (LibreOffice) → DOC
        ...
        ConverterProgress.Write(60, "LibreOffice 로 DOCX → DOC 변환 중");
        var progress = new Progress<(int Percent, string Message)>(t =>
            ConverterProgress.Write(t.Percent, t.Message));

        var docResult = await LibreOfficeBridge.ConvertFromDocxAsync(
            tmpDocxPath, tmpDir, "doc", sOfficePath: sOffice, progress: progress);

        File.Copy(docResult, tempOut, overwrite: true);
    }
...
{
    Console.WriteLine("PolyDonky.Convert.Doc — DOC ↔ IWPF 변환기");
    Console.WriteLine();
    Console.WriteLine("사용법:");
    Console.WriteLine("  PolyDonky.Convert.Doc <input> <output>");
    Console.WriteLine();
    Console.WriteLine("변환 쌍:");
    Console.WriteLine("  *.doc  → *.iwpf : import (LibreOffice → DOCX → IWPF)");
    Console.WriteLine("  *.iwpf → *.doc  : export (IWPF → DOCX → LibreOffice)");
    Console.WriteLine();
    ...
    Console.WriteLine("  예) set LIBREOFFICE_PATH=C:\\Program Files\\LibreOffice");
}
